...
@Component({
    selector: 'jhi-jerk-guoyi-suffix',
    templateUrl: './jerk-guoyi-suffix.component.html'
})
export class JerkGuoyiSuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/jerk-guoyi-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: JerkGuoyiSuffix) {
        return item.id;
    }
    registerChangeInJerks() {
        this.eventSubscriber = this.eventManager.subscribe('jerkListModification', (response) => this.loadAll());
    }
